...
def Motor(current):
    ...
    max spee
    '''
    motor_torque = current/10 # relationship from Fig 3.7
    ...
    return motor_torque, motor_rpm, motor_force, wheel_torque, wheel_rpm, max_motor_velocity

def Power(P_motor, P_Battery, P_SkinF, P_Drag, P_rr):
    P_resultant = P_Battery - (P_motor + P_Drag + P_SkinF + P_rr)
    return P_resultant

def Acceleration(F_rr, skin_friction, drag, motor_force):
    resultant_force = motor_force - F_rr - skin_friction - drag # sum the resistive forces
...
for i in range(3600):
    ...
    motor_torque, motor_rpm, motor_force, wheel_torque, wheel_rpm, max_motor_velocity= Motor(current)
    resultant_force, acceleration = Acceleration(F_rr, skin_friction, drag, motor_force)
    ...
    print(f"Resultant force: {resultant_force}, Acceleration: {acceleration}")
    print(f"Velocity: {velocity}, Distance: {distance}")
    print(drag, lift, skin_friction)
